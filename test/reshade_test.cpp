...
technique LeiFx_Tech
{
	...
	fxlang::EffectParser parse(str, fxlang::EffectType::Reshade);
	parse.Parse();

	auto& effect = parse.GetEffect();
	REQUIRE(effect.includes.size() == 2);
	REQUIRE(effect.uniforms.size() == 5);
	...
	glslang::TShader* shader = fxlang::ShaderParser::ParseHLSL(code);
	REQUIRE(shader != nullptr);

	auto root = shader->getIntermediate()->getTreeRoot();
	glslang::TIntermAggregate* func_node 
		= fxlang::ShaderInfo::GetFunctionByName("PS_3DFX1", *root);
	...
	std::deque<fxlang::Symbol> symbols;
	fxlang::ShaderInfo::FindSymbolsUsage("PS_3DFX2", *root, symbols);
...
technique Cartoon
{
	...
	auto& effect = parse.GetEffect();
	REQUIRE(effect.includes.size() == 2);
	REQUIRE(effect.uniforms.size() == 2);
	REQUIRE(effect.techniques.size() == 1);
	REQUIRE(effect.techniques[0].name == "Cartoon");
	...
	auto root = shader->getIntermediate()->getTreeRoot();
	glslang::TIntermAggregate* func_node 
		= fxlang::ShaderInfo::GetFunctionByName("CartoonPass", *root);
	REQUIRE(func_node != nullptr);
	...
	std::deque<fxlang::Symbol> symbols;
	fxlang::ShaderInfo::FindSymbolsUsage("CartoonPass", *root, symbols);
